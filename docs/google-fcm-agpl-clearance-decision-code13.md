# Kit Pay Android distribution clearance decision record — code 13

- Document type: Third-Party License Compatibility Review
- Decision classification: Internal Distribution Clearance Approval
- Approval reference: `KIT-PAY-AGPL-GOOGLE-FCM-CLEARANCE-2026-0722-003`
- Application: `com.kit.wallet`
- Product: Kit Pay
- Candidate version: `0.2.1` (`versionCode` 13)
...
reviewed the code-13 candidate identity, runtime dependency inventory, third-party
notices, applicable terms, distribution architecture, and the material encrypted-media
...
Code 13 adds bounded end-to-end encrypted photo messages to the existing Signal-based
direct-message protocol. Photo plaintext and attachment key material remain inside the
end-to-end encrypted client boundary. The server stores opaque ciphertext and
authenticated routing metadata, enforces sender ownership and exact metadata binding,
requires every active roster device to support code 13, and applies bounded unclaimed
and rolling upload quotas, expiry cleanup, and per-actor download throttles.

The Android client adds bounded picker/download reads, authenticated descriptor/wire
metadata reconciliation, off-main bounded image decoding, and a bounded zeroizing
plaintext media cache. These changes do not add or remove a third-party runtime
dependency, change a reviewed third-party licence, change the FCM adapter boundary,
change the libsignal version or licence, or change the AGPL corresponding-source
distribution model.

Decision `KIT-PAY-AGPL-GOOGLE-FCM-CLEARANCE-2026-0721-002` remains preserved for the
immutable code-12 candidate. This record does not move or reuse the code-12 identity or
tag.
...
`org.signal:libsignal-client:0.97.4` under `AGPL-3.0-only`. Code 13 does not change
...
- keeps attachment plaintext and key material outside the server boundary; and
- gates attachment delivery on exact metadata binding and all-device compatibility.
...
obligations, and the code-13 architecture changes. The runtime dependency and licence
boundaries remain those already reviewed for code 12. Distribution is approved under
the Company's internal release policy and the conditions below.
...
7. Direct sideload publication requires exact-APK signing and physical-device
   installation/launch evidence distinct from Play-delivered evidence.
8. Any material change identified by the internal release approval policy requires a
...
- [ ] Exact direct APK physical-device installation/launch evidence retained before
      `app.apk` publication
...
- Jurisdiction: Uganda
- Approval reference identifier: `KIT-PAY-AGPL-GOOGLE-FCM-CLEARANCE-2026-0722-003`
- Review date: 2026-07-22
...
The reviewed Kit Pay Android code-13 application may proceed with distribution under
