...
object Dest {
    ...
    const val SCHEDULED_CALLS = "calls/scheduled"
    const val CALL_INVITE_PREVIEW = "calls/invitation/{inviteToken}"
    ...
    const val CALL_INVITE = "call/invite/{inviteToken}?video={video}"
    ...
    fun callInvitePreview(token: String) = "calls/invitation/${Uri.encode(token)}"
    fun callInvite(token: String, video: Boolean) = "call/invite/${Uri.encode(token)}?video=$video"
